...
function raysgrid_Shortcode( $alias ) {
    ...
    $tax = $shonums = $ppp = $slider_atts = $output = $witbg = $mainbg = '';
        ...
        if ( $maximum_entries != 0 ) {
                                        ...
                                        $output .= '<div class="meta">';
                                            $output .= '<ul class="list">';
                                                ...
                                                if ($show_categories == '1') {
                                                    $output .= '<li><i class="fa fa-folder-open-o main-color"></i> <strong>' . esc_html__( 'Categories', PLUGIN_SLUG ) . ': </strong>';
                                                        $output .= implode ( ' , ', $termTax );
                                                    $output .= '</li>';
                                                }
                                                
                                                $output .= '<li><i class="fa fa-user main-color"></i> <strong>' . esc_html__('By', PLUGIN_SLUG ) . ' :</strong> 
                                                <a href="' . esc_url( get_author_posts_url( get_the_author_meta('ID') ) ) . '">' . get_the_author_meta( 'display_name' ) . '</a></li>';
                                                
                                                $output .= '<li><i class="fa fa-clock-o main-color"></i> <strong>' . esc_html__( 'Created on', PLUGIN_SLUG ) . ': </strong> ' . get_the_date() . '</li>';
            ...
            if ( $grid_layout != 'slider' && $pagination_type == 'smart' ) {
                 if ($pagination_style == 'loadmore') {
                    ...
                    $output .= '<div class="hidden pgnum">' . $query->max_num_pages . '</div>';
                        $output .='<span class="rsgd_load_more_scrl" data-num="2">' . esc_html__('Loading More ',PLUGIN_SLUG) . '<span class="rem_items">'. $totals .'</span>'. esc_html__(' Items',PLUGIN_SLUG) .'
            ...
            $output .= '<div class="alert alert-warning t-center m-b-0">' . esc_html__('No Posts To Show!!', PLUGIN_SLUG) . '</div>';
        }
        
        wp_reset_query();
        
    } else {
        $output .= '<div class="alert alert-danger t-center m-b-0">' . esc_html__('No grid with alias ( ' . esc_attr($alias) . ' ) were found!, Please Create it first.', PLUGIN_SLUG ) . '</div>';
    }
